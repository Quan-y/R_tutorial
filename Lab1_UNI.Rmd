...
sample_mean <- mean(normal100)
sample_std <- sd(normal100)
cat("The 100 standard noramlly-distributed is (sample 10) ", normal100[1:10])
cat("The sample mean is ", sample_mean)
cat("The sample std is ", sample_std)
# Answer: The mean and std not exactly be 0 and 1 because the sample size is not large enough 
# and they are randomly extracted from the distribution
```
...
```{r}
# hist plot
hist(normal100, freq = FALSE, col = 'black', nclass = 10)
```
...
```{r}
# Generate 10, 1000, 10,000, and 100,000 random draws from the standard normal distribution
...
cat("The content of the sample_means is ", sample_means, seq = '\n')
cat("The length of the sample_means is ", length(sample_means))
# Answer: the means become closer to 0 as the sample size increasing
```
...
```{r}
normal1mil = rnorm(1000000, mean = 3, sd = 4)
normal1mil_mean = mean(normal1mil)
normal1mil_sd = sd(normal1mil)
cat('The mean is', normal1mil_mean, '\nThe std is ', normal1mil_sd)
```
...
```{r}
new_normal1mil <- normal1mil
new_normal1mil <- new_normal1mil[new_normal1mil > 3]
cat("the mean of all the entries that are greater than 3 is ", mean(new_normal1mil))
```
...
```{r}
mean_col1234 = mean(normal1mil_mat[,1234])
cat("the mean of the $1234^{th}$ column is ", mean_col1234)
```
...
```{r}
# normal1mil_colmean = colSums(normal1mil_mat)/100
normal1mil_colmean = colMeans(normal1mil_mat)
```
...
```{r}
hist(normal1mil_colmean, col = 'black')
# Answer: Based on the shape of histogram, we could draw a conclusion that the normal1mil_colmean follows the normal distribution
```
