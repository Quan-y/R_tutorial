...
```{r}
plot(seq(0.001, 6, 0.01), f1(seq(0.001, 6, 0.01)), main = '-log(x)/(1+x)', xlab = 'x', ylab = 'y')
```


3)  By inspection, were do you think global minimum is located at?  
```{r}
# global minimum is located at x > 1 since when x < 1, the function value must be positive
```
...
```{r}
plot(seq(0.001, 6, 0.01), f1(seq(0.001, 6, 0.01)), main = '-log(x)/(1+x)', xlab = 'x', ylab = 'y', col = 1)
lines(seq(0.001, 6, 0.01), diff.quot(seq(0.001, 6, 0.01), 0.0001), col = 2)
```
...
```{r}
basic.grad.descent <- function (x, max.iter = 10000, stop.deriv = 1e-10, 
                                h = 0.0001, step.scale = 0.5){
  iter <- 0
  deriv <- Inf
  for (i in 1:max.iter) {
    iter <- iter + 1
    deriv <- diff.quot(x, h)
    error <- f1(x) - f1(x - deriv*step.scale)
    x <- x - deriv*step.scale
    if (error < stop.deriv) {break()}
    }
  fit <- list(x = x, minf = f1(x), iteration = iter,
