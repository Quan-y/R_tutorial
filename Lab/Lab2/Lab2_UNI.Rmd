...
cat('Coefficient of carat: ', lm0$coefficients[2])
cat('Intercept: ', lm0$coefficients[1])
```
...
n <- dim(diamonds)[1]
cat('The number of rows in the diamonds dataset: ', n)
```
...
for (b in 1:B) {
  data <- diamonds[resampled_values[b,],]
  ests <- lm(data$price~data$carat)$coefficients
...
hist(resampled_ests[,2], main = 'histogram of the bootstrap estimates of slope', xlab = 'slope estimate', ylab = 'frequency')
```
...
sd_slope = sd(resampled_ests[,2])
sd_intercept = sd(resampled_ests[,1])
cat('the standard deviation of the slope', sd_slope)
cat('the standard deviation of the intercept', sd_intercept)
```
...
Cl <- 2*lm0[[1]][[2]]-quantile(resampled_ests[,2], probs = 0.975)
Cu <- 2*lm0[[1]][[2]]-quantile(resampled_ests[,2], probs = 0.025)
int <- c(Cl, Cu)
int
```
